...
<Accordion title="Span Grouping for ThreadpoolExecutor">
  Please use the provided code snippet as an example to ensure that the current tracing context is properly propagated when executing tasks within a `ThreadPoolExecutor`. The `run_with_context` function attaches the given context before running the specified function (func) and detaches the context after the function execution, ensuring that the tracing information is maintained. The ThreadPoolExecutor is used to run multiple tasks in parallel, each within the context captured in current_context. This approach ensures that all spans created within the threaded tasks are part of the root span, maintaining the trace's integrity.
...
from opentelemetry.context import get_current, attach, detac

# flake8: noqa
from langtrace_python_sdk import langtrace, with_langtrace_root_spa
langtrace.init(api_ky="your-langtrace-api-key")
...
def example():
  ...
  # Get the current tracing context and run the optimized CoT on each question in parallel
  ...
  with ThreadPoolExecutor(max_workers=2) as executor:
    # Run the optimized CoT on each question in parallel by passing the current tracing context
