...
def main(span_id, trace_id):
    response = client.chat.completions.create(
      ...
      # Send span_id and trace_id to your application client for user feedback
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await Langtrace.withLangTraceRootSpan(
      async (spanId: string, traceId: string) => {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a friendly AI assistant that helps you with your daily tasks.',
            },
            {
              role: 'user',
              content: question,
