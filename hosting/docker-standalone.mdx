...
```bash
docker run -d -p 3000:3000 --env-file .env --name langtrace scale3labs/langtrace-client /bin/sh -c "npm run create-tables && npm run dev"
```

The Langtrace app will be available at `http://localhost:3000`

<Tip>
  If you wish to manually pull the specific version of the Docker image from docker hub, you can use the following command
